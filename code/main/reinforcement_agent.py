...
class Network(nn.Module):
    def __init__(self, HIDDEN_LAYER, NUM_OUTPUTS):
        nn.Module.__init__(self)
        # self.l1 = nn.Linear(5, HIDDEN_LAYER)
        # self.l2 = nn.Linear(HIDDEN_LAYER, NUM_OUTPUTS*2) # One output node for calculating the EV from incrementing OR decrementing each of the 4 modulation params.
        self.l1 = nn.Linear(5, NUM_OUTPUTS*2)

    def forward(self, x):
        # x = F.relu(self.l1(x))
        # x = self.l2(x)

        x = self.l1(x) # 1-layer version
        return x


class DQN_Agent:
    def __init__(self, NUM_OUTPUTS, EPS_START=0.9, EPS_END=0.05, EPS_DECAY=80, GAMMA=0.9, LR=0.1, HIDDEN_LAYER=2, BATCH_SIZE=30, MEM_CAP=600):
        ### Hyper Parameters ###
        ...
        self.model = Network(self.HIDDEN_LAYER, self.NUM_OUTPUTS)
    ...
    def select_action(self, state):
        ...
        RETURNS: `action`: A straight python list of ints that correponds to either incrementing or 
                           decrementing one of the 4 modulation values.
        """
        ...
        # THIS CODE BLOCK IS FOR IF WE WANT TO HAVE MULTIPLE ACTIONS PER TIME STEP
        ...
        #             two_long_np = output[i:i+2]
        #             res = np.where(two_long_np == np.amax(two_long_np))
        #             results.append(i+res[0][0])
        ...
        #     results = []
        #     for i in range(4):
        #         results.append(i*2+random.randint(0, 1))
        #     return results
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                # found, so we pick action with the larger expected reward.
                output = self.model(torch.Tensor(state))
                print("FROM THE DQN: {}".format(output))
                output = output.detach().numpy()
                result = np.where(output == np.amax(output))
                print("PROCESSED: {}".format(int(result[0][0])))
                return int(result[0][0])
                
        else: 
            print('Random choice...')
            return random.randint(0,(self.NUM_OUTPUTS*2)-1)
    ...
    def optimize_model(self):
        ...
        # print(torch.transpose(expected_q_values.unsqueeze(0), 1, 0))
        # loss is measured from error between current and newly expected Q values
        loss = F.smooth_l1_loss(current_q_values, torch.transpose(expected_q_values.unsqueeze(0), 1, 0))
